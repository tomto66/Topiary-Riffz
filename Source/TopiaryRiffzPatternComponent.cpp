...
void TopiaryRiffzPatternComponent::actionListenerCallback(const String &message)
{
	...
	else if (message.compare(MsgPatternList) == 0)
	{
		...
		patternCombo.clear();
		for (int i = 0; i < MAXNOPATTERNS; i++) patterns[i] = "";
...
void TopiaryRiffzPatternComponent::addNote()
{
	...
	{
		riffzModel->getNote(patternCombo.getSelectedId() - 1, patternTable.getSelectedRow(), note, velocity, timestamp, length, midiType, value);
	}
...
void TopiaryRiffzPatternComponent::addAT()
{
	// adds a note at the position selected in table
	int timestamp = 0;
	int note = 0;
	...
	riffzModel->addAT(patternCombo.getSelectedId() - 1, 0, timestamp);
...
void TopiaryRiffzPatternComponent::addPitch()
{
	// adds a note at the position selected in table
	int timestamp = 0;
	int note = 0;
	...
	riffzModel->addPitch(patternCombo.getSelectedId() - 1, 0, timestamp);
...
void TopiaryRiffzPatternComponent::addCC()
{
	// adds a note at the position selected in table
	int timestamp = 0;
	int note = 0;
	...
	riffzModel->addCC(patternCombo.getSelectedId() - 1, 1, 0, timestamp);
...
void TopiaryRiffzPatternComponent::copyNote()
{
	...
	riffzModel->getNote(patternCombo.getSelectedId() - 1, patternTable.getSelectedRow(), pbufferNote, pbufferVelocity, timestamp, pbufferLength, pMidiType, pValue);

	// call setButtonStates so paste button comes on
	setButtonStates();
...
void TopiaryRiffzPatternComponent::pasteNote()
{
	// adds a note at the position selected in table
	int timestamp = 0;
	int length, note, velocity, midiType, value;
	...
	if (pMidiType == Topiary::NoteOn)
		riffzModel->addNote(patternCombo.getSelectedId() - 1, pbufferNote, pbufferVelocity, pbufferLength, timestamp);
	else if (pMidiType == Topiary::AfterTouch)
		riffzModel->addAT(patternCombo.getSelectedId() - 1, pValue, timestamp);
	else if (pMidiType == Topiary::CC)
		riffzModel->addCC(patternCombo.getSelectedId() - 1, pbufferNote, pValue, timestamp);
	else if (pMidiType == Topiary::Pitch)
		riffzModel->addPitch(patternCombo.getSelectedId() - 1, pValue, timestamp);
