...
public:
	...
	String name = "New Riffz Pattern"; // name of this preset
...
private:
		   ...
		   if ( ((*pat).dataList[pIndex].measure == measureToGenerate) || (measureToGenerate == -1) )
		   {
			...
			if (poolNote[note] == p) // note in this pool
			{
				...
				if (variation[v].swing && variation[v].swingPool[p] && doNote)
				{
					...
					//int debugSecond;
					//int debugThird;
					if (variation[v].swingQ == TopiaryRiffzModel::SwingQButtonIds::SwingQ8)
					{
	...
	{ 
		...
		if (q == TopiaryRiffzModel::SwingQButtonIds::SwingQ8)
