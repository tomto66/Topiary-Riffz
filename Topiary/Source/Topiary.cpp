...
String validateNote(String newNote)
{
	...
	// only for midi notes 21-108 (88 key keyboard)
	// So A1 -- C9

	String validatedNote;
		
	int noteNumber = validNoteNumber(newNote);
	if (noteNumber == -1) validatedNote = "C2";
	else if (noteNumber > 108) validatedNote = "C9";
	else if (noteNumber < 21) validatedNote = "A1";
	else validatedNote = newNote.toUpperCase();
...
int validNoteNumber(String inNote)
{
	...
	inNote = inNote.toUpperCase();

	for (int i = 0; i < 12; i++)
	{
		if (inNote.indexOfIgnoreCase(notes[i]) == 0)
		{
			noteFound = i;
			String offsetS = inNote.replace(notes[i], "");

			int offset = offsetS.getIntValue();
			 
			noteNumber = (offset * 12) + i;
			i = 12;
		}
	}
